...
async def setup_guild(client: "BotClient") -> None:
    ...
    if not isinstance(client.guild, Guild):
...
async def create_roles(client: "BotClient", guild: Guild) -> None:
    ...
    if discord.utils.get(guild.roles, name=ADMIN_ROLE) is None:
        client.admin_role = await guild.create_role(name=ADMIN_ROLE, mentionable=True,
                                                    colour=blueshirt_blue, permissions=PERMISSIONS['admin'])
    if discord.utils.get(guild.roles, name=VOLUNTEER_ROLE) is None:
        client.volunteer_role = await guild.create_role(name=VOLUNTEER_ROLE, mentionable=True, hoist=True,
                                                        colour=blueshirt_blue, permissions=PERMISSIONS['blueshirt'])
    if discord.utils.get(guild.roles, name="Robots") is None:
        robots = await guild.create_role(name="Robots", mentionable=True,
                                         hoist=True, colour=discord.Colour.from_str("#607d8b"))
    ...
    if discord.utils.get(guild.roles, name="Team Supervisor") is None:
        client.supervisor_role = await guild.create_role(name="Team Supervisor", mentionable=False,
                                                         hoist=True, colour=discord.Colour.from_str("#e74c3c"))
    if discord.utils.get(guild.roles, name="Team Support") is None:
        await guild.create_role(name="Team Support", mentionable=False,
                                hoist=True, colour=discord.Colour.from_str("#992d22"))
    if discord.utils.get(guild.roles, name=SPECIAL_ROLE) is None:
        client.special_role = await guild.create_role(name=SPECIAL_ROLE, mentionable=False)
    if discord.utils.get(guild.roles, name=VERIFIED_ROLE) is None:
        client.verified_role = await guild.create_role(name=VERIFIED_ROLE, mentionable=False,
...
async def create_channels(client: "BotClient", guild: Guild) -> None:
    ...
    for change in diff:
        if change.requires_community and "COMMUNITY" not in guild.features:
            ...
            if isinstance(rules_channel, TextChannel) and isinstance(public_updates_channel, TextChannel):
                await guild.edit(
        ...
        if isinstance(channel, TextChannel):
            ...
            for index, message in enumerate(messages):
                if channel == client.blueshirt_onboarding_channel and "Step 3" in message:
                    result = await post_message(channel, message, BlueshirtConfirmView())
                else:
